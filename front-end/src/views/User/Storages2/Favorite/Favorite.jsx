    ...
    useEffect(() => {
        (async () => {
            ...
            if (res.status === "success") {
                dispatch({ type: storageActionTypes.setStorageData, payload: { child_folders: res.data.favorited_folders, child_files: res.data.favorited_files, path: [] } });
            } else {
    ...
    return (
        <Container className="my_storage" style={{ maxWidth: "1605px", margin: "0 auto" }}>
            <div className="my_storage_header">
                <div className="my_storage_header_title">Favorite</div>
            </div>
