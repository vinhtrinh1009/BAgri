...
export default function Recent() {
    ...
    useEffect(() => {
        (async () => {
            const res = await storageService.getRecents();
            if (res.status === "success") {
                dispatch({ type: storageActionTypes.setStorageData, payload: { child_folders: res.data.recent_folders, child_files: res.data.recent_files, path: [] } });
            } else {
    ...
    return (
        <Container className="my_storage" style={{ maxWidth: "1605px", margin: "0 auto" }}>
            <div className="my_storage_header">
                <div className="my_storage_header_title">Recent</div>
            </div>
