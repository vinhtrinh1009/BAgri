...
from exceptions import DigitalOceanRequestError
...
def get_slug_version(digital_ocean_token):
    ...
    try:
        version = json.loads(response.content.decode("utf-8"))["options"]["versions"][
            -1
        ]["slug"]
        return version
    except (KeyError, IndexError) as e:
        _LOGGER.debug(f"Cannot get cluster slugversion in digital ocean.")
        raise DigitalOceanRequestError(f"Cannot get cluster slug verion in DigitalOcean")
...
def create_cluster(name, number_nodes, cpu, ram, digital_ocean_token):
    ...
    body_request = {
        ...
        "node_pools": [
            {
                "size": "s-" + str(cpu) + "vcpu-" + str(ram) + "gb",
                "count": int(number_nodes),
                "name": "fabric-pool-" + name,
                "tags": ["fabric-pool"],
            }
        ],
    }
    ...
    try:
        ...
        _LOGGER.debug(
            f"Cannot create cluster in digital ocean. Response: {response_content}"
        )
        raise DigitalOceanRequestError("Cannot create cluster in digital ocean")
...
def get_cluster_status(cluster_id, digital_ocean_token):
    ...
    try:
        status = json.loads(response.content.decode("utf-8"))["kubernetes_cluster"][
            "status"
        ]["state"]
        return status
    except KeyError:
        raise DigitalOceanRequestError(
            f"Cannot get status of cluster {cluster_id} in DigitalOcean"
        )
...
def delete_cluster(cluster_id, digital_ocean_token):
    ...
    if response.status_code != 204:
        raise DigitalOceanRequestError(
            f"Cannot delete k8s cluster {cluster_id} in DigitalOcean"
        )
...
def get_kubeconfig(cluster_id, digital_ocean_token):
    ...
    while response_get.status_code != 200:
        ...
        if time.time() - time_start >= 60:
            raise DigitalOceanRequestError(
                f"Cannot retrieve kubeconfig of cluster {cluster_id}"
            )
