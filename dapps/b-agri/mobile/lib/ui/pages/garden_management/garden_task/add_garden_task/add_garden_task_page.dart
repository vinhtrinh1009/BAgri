...
import 'add_garden_task_cubit.dart';

class AddGardenTaskPage extends StatefulWidget {
  final String? name;
  final String? season_id;
  AddGardenTaskPage({Key? key, this.name, this.season_id}) : super(key: key);

  @override
  _AddGardenTaskPageState createState() => _AddGardenTaskPageState();
}

class _AddGardenTaskPageState extends State<AddGardenTaskPage> {
  ...
  AppCubit? _appCubit;
  late AddGardenTaskCubit _cubit;

  @override
  void initState() {
    super.initState();
    _appCubit = BlocProvider.of<AppCubit>(context);
    _cubit = BlocProvider.of<AddGardenTaskCubit>(context);
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    AppPageStepPicker(
                      ...
                      controller: StepPickerController(),
                      onChanged: (value) {
                    ...
                    SizedBox(height: 10),
                    AppPageFarmerPicker(
                      controller: FarmerPickerController(),
                      onChanged: (value) {
  ...
  Widget _buildPickFileButton() {
    return BlocBuilder<AddGardenTaskCubit, AddGardenTaskState>(
      bloc: _cubit,
  ...
  Widget _buildActionCreate(BuildContext context) {
    return BlocConsumer<AddGardenTaskCubit, AddGardenTaskState>(
      bloc: _cubit,
      listenWhen: (prev, current) {
        return prev.addGardenTaskStatus != current.addGardenTaskStatus;
      },
      listener: (context, state) {
        if (state.addGardenTaskStatus == LoadStatus.SUCCESS) {
          _showCreateSuccess();
        }
        if (state.addGardenTaskStatus == LoadStatus.FAILURE) {
          showSnackBar('Có lỗi xảy ra!');
        }
      },
      builder: (context, state) {
        final isLoading = (state.addGardenTaskStatus == LoadStatus.LOADING);
        return Row(
...
class GardenTaskCreateArgument {
  String? name;
  String? season_id;

  GardenTaskCreateArgument({
    this.name,
    this.season_id,
  });
}
